...
if (isset($_GET['wp_easy_events_optin'])) {
	if (!function_exists('wp_get_current_user')) {
		require_once (ABSPATH . 'wp-includes/pluggable.php');
	}
	if ($_GET['wp_easy_events_optin'] == 1) {
		//opt-in
	...
	wp_redirect(esc_url(remove_query_arg('wp_easy_events_optin')));
	exit;
} else {
...
function wp_easy_events_show_optin() {
	...
	if (!get_option('wp_easy_events_tracking_optin')) {
		...
		$tr_link = implode(' ', array(
			sprintf(__('%sDo not allow%s', 'wp-easy-events') , '<a href="' . admin_url('admin.php?page=wp_easy_events&wp_easy_events_optin=0') . '" class="button-secondary" id="wp-easy-events-do-not-allow-tracking">', '</a>') ,
			sprintf(__('%sAllow%s', 'wp-easy-events') , '<a href="' . admin_url('admin.php?page=wp_easy_events&wp_easy_events_optin=1') . '" class="button-primary" id="wp-easy-events-allow-tracking">', '</a>') ,
		));
		...
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . __('Newsletter', 'wp-easy-events') . '</span><p>' . __('Updates, announcements, marketing, no spam', 'wp-easy-events') . ', <a href="https://emdplugins.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
		echo '</ul></div></div>';
	} else {
...
function wp_easy_events_show_rateme_action() {
	...
	if (10 === $min_trigger) {
		...
		$response['redirect'] = "https://emdplugins.com/plugin_tag/wp-easy-events";
		$min_trigger = - 1;
	}
...
function wp_easy_events_show_rateme_notice() {
	...
	if ($min_count > 10 || ($min_trigger == 10 && $installed_date <= $today)) {
		$message_start = '<div class="emd-show-rateme update-nag success">
                        <span class=""><b>WP Easy Events</b></span>
                        <div>';
		if ($min_count > 5) {
			$message_start.= sprintf(__("Hi, I noticed you just crossed the %d %s milestone on WP Easy Events - that's awesome!", "wp-easy-events") , $min_trigger, $label);
		...
		$message_level1 = __('Could you please do me a <span style="color:red" class="dashicons dashicons-heart"></span> BIG favor <span style="color:red" class="dashicons dashicons-heart"></span> and give it a 5-star rating on WordPress? Just to help us spread the word and boost our motivation.', "wp-easy-events");
		...
		$message_end1 = '<li><a data-rate-action="do-rate" data-plugin="wp_easy_events" href="#">' . __('Ok, you deserve it', 'wp-easy-events') . '</a>
       </li>
        <li><a data-rate-action="done-rating" data-plugin="wp_easy_events" href="#">' . __('I already did', 'wp-easy-events') . '</a></li>
...
function wp_easy_events_plugin_row_meta($input, $file) {
	...
	$links = array(
		...
		'<a href="https://emdplugins.com/plugin_tag/wp-easy-events">' . __('Pro Version', 'wp-easy-events') . '</a>'
	);
...
function wp_easy_events_deactivation_feedback_box() {
	...
	foreach ($reasons as $key => $reason) {
		...
		switch ($key) {
			...
			case 9:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share your previous version number before update, new updated version number and what happened', 'wp-easy-events') . '"';
			break;
			case 5:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share what happened', 'wp-easy-events') . '"';
			break;
			case 22:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
