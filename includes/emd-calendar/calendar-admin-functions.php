...
function emd_calendar($atts){
	...
	if(isset($calendar_conf['jui_theme']) && $calendar_conf['jui_theme'] != 'none'){
		...
		wp_enqueue_style('theme-jui-css', constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/css/jquery-ui-themes-1.11.4/' . $jui_theme . '/jquery-ui.min.css');
		wp_enqueue_style('theme-css', constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/css/jquery-ui-themes-1.11.4/' . $jui_theme . '/theme.css');
	}

	wp_enqueue_style('calendar-css', constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/css/fullcalendar.min.css');
	wp_enqueue_script('moment-js',constant(strtoupper($app_name) . '_PLUGIN_URL') .'includes/emd-calendar/js/moment.min.js');
	wp_enqueue_script('fullcalendar-js',constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/js/fullcalendar.min.js');
	wp_enqueue_script('emd-calendar-js', constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/js/emd-calendar.js');
	wp_localize_script( 'emd-calendar-js', 'calendar_vars', $calendar_vars);
	wp_enqueue_style('emd-calendar-css', constant(strtoupper($app_name) . '_PLUGIN_URL') . 'includes/emd-calendar/css/emd-calendar.css');
