...
add_action('wp_easy_events_upgrade', 'wp_easy_events_upgrade_ver_2_5');
function wp_easy_events_upgrade_ver_2_5($new_version) {
	if ($new_version == '2.5.0') {
		...
		if (!empty($postslist)) {
			foreach ($postslist as $mypost) {
				$feat_id = get_post_thumbnail_id($mypost->ID);
				if (empty($feat_id)) {
					$att_ids = get_post_meta($mypost->ID, 'emd_event_picture', false);
					if (!empty($att_ids) && !empty($att_ids[0])) {
						set_post_thumbnail($mypost->ID, $att_ids[0]);
					}
				}
			}
		}
		...
		if (sizeof($payment->downloads) > 0) {
			...
			if (!empty($myorder)) {
				foreach ($myorder as $pr_id => $qty) {
					...
					$connected = p2p_type('emd_wpe_event_edd_product')->get_connected($pr_id);
					if (!empty($connected->posts)) {
						$event_id = $connected->posts[0]->ID;
						//3- create relationship with event and order
						$p2p_id = p2p_type('emd_wpe_event_edd_order')->get_p2p_id($payment_id, $event_id);
						if (empty($p2p_id)) {
							p2p_type('emd_wpe_event_edd_order')->connect($payment_id, $event_id, array(
	...
	if (class_exists('Wp_Easy_Events_WooCommerce') && class_exists('WooCommerce')) {
		...
		if (sizeof($order->get_items()) > 0) {
			...
			if (!empty($myorder)) {
				foreach ($myorder as $pr_id => $qty) {
					//2- see if product is related to an event
					$connected = p2p_type('emd_wpe_event_woo_product')->get_connected($pr_id);
					if (!empty($connected->posts)) {
						$event_id = $connected->posts[0]->ID;
						//3- create relationship with event and order
						$p2p_id = p2p_type('emd_wpe_event_woo_order')->get_p2p_id($order_id, $event_id);
						if (empty($p2p_id)) {
							p2p_type('emd_wpe_event_woo_order')->connect($order_id, $event_id, array(
								'date' => current_time('mysql')
							));
