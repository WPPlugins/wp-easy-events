...
<div class="section-wrapper">
    <div style="padding-bottom:10px;clear:both" id="modified-info-block" class=" text-right">
        <div class="textSmall text-muted"><?php _e('Last modified by', 'wp-easy-events'); ?> <?php echo get_the_modified_author(); ?> - <?php echo human_time_diff(strtotime(get_the_modified_date() . " " . get_the_modified_time()) , current_time('timestamp')); ?> <?php _e('ago', 'wp-easy-events'); ?></div>
    </div>
    <div class="section-toolbar clearfix" style="margin-bottom:20px;width: 100%;">
        <div class="toolbar-btn pull-right"><?php echo emd_get_tax_vals(get_the_ID() , 'emd_event_cat'); ?></div>
        <div class="toolbar-btn calendar-export-btns pull-left">
    ...
    <div class="section-heading">
        <div class='single-title font-bold header'>
            <h2 class='entry-title'>
                <span class="emd_wpe_event-title"><?php echo get_the_title(); ?></span>
            </h2>
        </div>
    </div>
    <div class="section-body" style="clear:both">
        <div class="single-well well emd-wpe-event">
            <div class="row">
                <div class="col-sm-<?php if (emd_is_item_visible('featured_img', 'wp_easy_events', 'attribute')) {
	echo '12 well-left';
} else {
	echo ' hidden';
} ?>">
                    <div class="slcontent emdbox">
                        <?php if (emd_is_item_visible('featured_img', 'wp_easy_events', 'attribute')) { ?>
                        <div class="img-gallery segment-block featured-img"><a title="<?php echo get_the_title(); ?>" href="<?php echo wp_get_attachment_url(get_post_thumbnail_id()); ?>"><?php echo get_the_post_thumbnail(null, 'large'); ?></a></div>
                        <?php
} ?>
                    </div>
                </div>
	...
	echo '12 well-right';
                    ...
                    <div class="srcontent emdbox">
                        ...
                        <?php
} ?><?php if (emd_is_item_visible('ent_event_cost', 'wp_easy_events', 'attribute')) { ?>
                        <div class="single-content segment-block ent-event-cost"><?php echo esc_html(emd_mb_meta('emd_event_cost')); ?>
</div>
                        <?php
} ?><?php if (emd_is_item_visible('ent_event_external_url', 'wp_easy_events', 'attribute')) { ?>
                        <div class="single-content segment-block ent-event-external-url">🔗 <a target='_blank' href='<?php echo esc_html(emd_mb_meta('emd_event_external_url')); ?>
' title='<?php echo get_the_title(); ?>'><?php _e('Click for more info', 'wp-easy-events'); ?></a></div>
                        <?php
} ?><?php $modal_content = '';
...
if (!empty($modal_content) && !empty(emd_mb_meta('emd_event_registration_type')) && emd_mb_meta('emd_event_registration_type') != 'none') {
	echo ' 
                        <div class="section-row" style="margin:10px 0"> <a class="export-button" href="#signup" id="go" name="signup" rel="leanModal">';
	echo $modal_button;
	echo '</a> </div>
                        <!-- Modal -->
                        <div id="signup">
	...
	echo '<a href="#" class="modal_close"></a></div>
        ...
        <div class="section-subbody" style="clear:both">
            <?php if (emd_is_item_visible('content', 'wp_easy_events', 'attribute')) { ?>
            <div class="single-content content"><?php echo $post->post_content; ?></div>
            <?php
} ?>
        </div>
        <div class="rel-group" id="relwrapper"> <?php if (emd_is_item_visible('entrelcon_event_organizer', 'wp_easy_events', 'relation')) { ?>
<?php $post = get_post();
	$rel_filter = "";
	$res = emd_get_p2p_connections('connected', 'event_organizer', 'std', $post, 1, 0, '', 'wp_easy_events', $rel_filter);
	if (!empty($res['rels'])) {
?>
<div class="emd-wpe-event entrelcon-event-organizer"><div class="relation-title" style="clear:both"><h3><div class="accor-title"><?php _e('Organizers', 'wp-easy-events'); ?></div></h3></div> <?php
...
<div class="relblock">
    <ul style="padding-left:0;list-style:none;" class="emd-rellist">
        <li class="emd-rellist-permalink"> <a href="<?php echo get_permalink($post->ID); ?>" title="<?php echo get_the_title(); ?>"><?php echo get_the_title(); ?></a> </li>
        <?php if (emd_is_item_visible('featured_img', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="featured-img segment-block">
                <span class="segvalue"><a title="<?php echo get_the_title(); ?>" href="<?php echo get_permalink($post->ID); ?>"><?php echo get_the_post_thumbnail(null, 'thumbnail'); ?></a></span>
            </div>
        </li>
        <?php
			} ?> <?php if (emd_is_item_visible('ent_eo_email', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="ent-eo-email segment-block">
                <span class="segvalue emd-thumb">✉ <a href="mailto:<?php echo antispambot(emd_mb_meta('emd_eo_email')); ?>"><?php echo antispambot(emd_mb_meta('emd_eo_email')); ?></a></span>
            </div>
        </li>
        <?php
			} ?> <?php if (emd_is_item_visible('ent_eo_phone', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="ent-eo-phone segment-block">
                <span class="segvalue emd-thumb">&phone; <a href="tel:<?php echo esc_html(emd_mb_meta('emd_eo_phone', '', $myrel->ID)); ?>"><?php echo esc_html(emd_mb_meta('emd_eo_phone', '', $myrel->ID)); ?></a></span>
            </div>
        </li>
        <?php
			} ?> <?php if (emd_is_item_visible('ent_eo_website', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="ent-eo-website segment-block">
                <span class="segvalue emd-thumb">🔗<a target='_blank' href='<?php echo esc_html(emd_mb_meta('emd_eo_website', '', $myrel->ID)); ?>' title='<?php echo get_the_title(); ?>'><?php _e('Click for more info', 'wp-easy-events'); ?></a></span>
            </div>
        </li>
        <?php
			} ?> 
...
<?php $post = get_post();
	...
	if (!empty($res['rels'])) {
?>
<div class="emd-wpe-event entrelcon-event-venue"><div class="relation-title" style="clear:both"><h3><div class="accor-title"><?php _e('Venue', 'wp-easy-events'); ?></div></h3></div> <?php
		echo $res['before_list'];
		$real_post = $post;
		$rel_count_id = 1;
		foreach ($res['rels'] as $myrel) {
...
<div class="relblock">
    <ul style="padding-left:0;list-style:none;" class="emd-rellist">
        ...
        <?php
			} ?> <?php if (emd_is_item_visible('ent_venue_fulladdress', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="ent-venue-fulladdress segment-block"> <span class="segvalue emd-thumb">📌 <?php echo esc_html(emd_mb_meta('emd_venue_fulladdress', '', $myrel->ID)); ?></span> </div>
        </li>
        <?php
			} ?> <?php if (emd_is_item_visible('excerpt', 'wp_easy_events', 'attribute')) { ?> 
        <li class=" relseg">
            <div class="excerpt segment-block"> <span class="segvalue emd-thumb"><?php echo $post->post_excerpt; ?></span> </div>
        </li>
        <?php
			} ?> 
    </ul>
...
<?php
    ...
    <div class="section-footer" style="clear:both">
        <?php if (emd_is_item_visible('tax_emd_event_tag', 'wp_easy_events', 'taxonomy')) { ?>
        <div class=""><span style="margin-right:2px" class="footer-object-title label label-info"></span><span class="footer-object-value"><?php echo emd_get_tax_vals(get_the_ID() , 'emd_event_tag'); ?></span></div>
        <?php
} ?> 
    </div>
</div>
